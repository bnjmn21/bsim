...
import { And, Or, Xor, Toggle, LED, Block, circuitPlugin, OutputNode, InputNode, BlockDef, Hitbox, NodeRef, Delay, WireNode, Not, getSelectedBlocks, deselectAll, Circuit } from "./blocks.js";
...
export type CreatingSelection = {type: "selection", start: Vec2};
export type MovingSelection = {type: "moveSelection", start: Vec2, blockPositions: Vec2[], initialSelFrame: {pos1: Vec2, pos2: Vec2}};
export const dragging: {inner: DraggingNewBlock | MovingBlock | DraggingWire | CreatingSelection | MovingSelection | null} = {
    inner: null, //Inner mutability
};

export const selectionFrame: {inner: {pos1: Vec2, pos2: Vec2} | null} = {
    inner: null
};
    ...
    world.system(Loop, [], _ => {
        perfData.render.other += timed(time, () => {
            if (dragging.inner?.type === "selection") {
                ...
                ctx.beginPath();
                ctx.roundRect(
                    dragging.inner.start.x,
                    dragging.inner.start.y,
                    camera.mouseWorldCoords().sub(dragging.inner.start).x,
                    camera.mouseWorldCoords().sub(dragging.inner.start).y,
                    5
                );
                ctx.fill();
                ctx.stroke();
            ...
            if (selectionFrame.inner !== null) {
                ...
                ctx.beginPath();
                ctx.roundRect(
                    selectionFrame.inner.pos1.x,
                    selectionFrame.inner.pos1.y,
                    selectionFrame.inner.pos2.sub(selectionFrame.inner.pos1).x,
                    selectionFrame.inner.pos2.sub(selectionFrame.inner.pos1).y,
                    5
                );
                ctx.stroke();
                ctx.restore();
...
function rectToRectHibox(rect1: {pos1: Vec2, pos2: Vec2}, rect2: {pos1: Vec2, pos2: Vec2}) {
...
function circleToRectHitbox(circle: {pos: Vec2, radius: number}, rect: {pos1: Vec2, pos2: Vec2}) {
...
export function hitboxToRect(hb: Hitbox, rect: {pos1: Vec2, pos2: Vec2}) {
    if (hb.type === "circle") {
